...
// a table size (which must be prime),
// and a function pointer to a hash function.
typedef struct hashmap_st {
    linkedlist_t **table;
    size_t tableSize;
    unsigned (*hashFunction)(size_t M, char *key);
} hashmap_t;
...
// Allocates a new hashmap and returns a pointer.
// The user must free the hashmap_t* with HashmapFree.
// Uses the provided hash function.
hashmap_t *HashmapCreateCustom(unsigned (*hashFunction)(size_t M, char *key));
